        ...
        public static void HandleRequests(WebApplication app)
        {
            ...
            async Task<string> UseYTDlP(string query)
            {
                ...
                if (EverythingEveryWhereAllAtOnce.LOG_A_TON)
                    Console.WriteLine("YAYTWCR: InnerTube Response:\n" + response.Data);
            ...
            app.MapGet(@"/feeds/api/videos/", async (HttpRequest request) => // needs to be ?q at some point for real hardware
            {
                ...
                if(EverythingEveryWhereAllAtOnce.LOG_A_TON)
                    Console.WriteLine("YAYTWCR: Query: " + query);

                string json = await UseYTDlP($"ytsearch20:{query}");
                (string, int) data = await Extract(json, request);

                string base_url = "{request.Scheme}://{request.Host}{request.PathBase}";
                string continuation = "placeholder";
                ...
                */
                ...
                {
                    Console.Error.WriteLine($"YAYTWCR: {ex}");
